...
<div class="relative rounded-lg flex flex-col md:flex-row items-center md:shadow-xl md:h-72 mx-2 my-6">
	...
	>
		<div
			...
			style="background-image: url( {image} ); background-blend-mode: multiply;"
		/>
		<div
		...
		>
			<h3 class="w-full font-bold text-2xl text-white leading-tight mb-2">{title}</h3>
		</div>
		<svg
	...
	<div class="z-10 order-2 md:order-1 w-full h-full md:w-3/5 flex items-center -mt-6 md:mt-0">
		...
		>
			<h3 class="hidden md:block font-bold text-2xl text-gray-700">{title}</h3>
